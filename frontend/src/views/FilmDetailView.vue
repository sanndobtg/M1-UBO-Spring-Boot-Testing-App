    ...
    <template v-else-if="film">
      ...
      <div class="film-hero">
        ...
        <div class="container hero-content">
          ...
          <div class="film-info">
            ...
            <div v-if="avgData" class="film-rating">
              <div class="rating-stars">
                <span v-for="i in 5" :key="i" class="star" :class="{ filled: i <= Math.round(avgData.moyenne) }">★</span>
              </div>
              <span class="rating-num neon-text">{{ avgData.moyenne?.toFixed(1) }}</span>
              <span class="rating-count">/ 5 — {{ avgData.nombre }} avis</span>
            </div>
            ...
            <div v-if="posters.length > 1" class="poster-thumbs">
      ...
      <div class="reviews-section">
        <div class="container">
          ...
          <!-- Formulaire avis (connecté + a loué) -->
          <div v-if="auth.isLoggedIn && (alreadyRented || hasRentedBefore)" class="review-form cyber-card">
          ...
          <div v-else-if="auth.isLoggedIn && !alreadyRented && !hasRentedBefore" class="review-info">
            <span class="info-icon">ℹ</span>
            Vous devez avoir loué ce film pour laisser un avis.
          </div>
        ...
        <div v-if="showPayModal" class="modal-overlay" @click.self="showPayModal = false">
          <div class="pay-modal">
            <div class="modal-header">
              <span class="modal-title">// PAIEMENT</span>
            ...
            <div class="modal-body">
              ...
              <button class="btn-pay" @click="confirmPayment" :disabled="paying">
...
onMounted(async () => {
  ...
  try {
    ...
    film.value = { id, titre: `Film #${id}`, annee: '—', genres: [], ouvert: false }
  } finally {
    loading.value = false
  }
...
async function loadAverage(titre) {
  try {
    const res = await reviewService.getAverage(titre)
    avgData.value = res.data
  } catch { avgData.value = null }
}
...
async function checkReservation(titre) {
  try {
    ...
    const all = res.data || []
    alreadyRented.value = all.some(r => r.titreFilm === titre || r.film?.titre === titre)
...
.poster-frame {
  ...
  overflow: hidden; box-shadow: var(--shadow-neon), var(--shadow-card);
}
...
.film-meta { display: flex; align-items: baseline; gap: 12px; flex-wrap: wrap; }
.meta-label { font-family: var(--font-mono); font-size: 0.6rem; letter-spacing: 0.2em; color: var(--neon-cyan); flex-shrink: 0; }
.meta-value { font-size: 0.95rem; color: var(--text-secondary); }
...
.btn-reserve, .btn-rented { padding: 14px 32px !important; }
.btn-rented { border-color: var(--neon-green) !important; color: var(--neon-green) !important; }

.closed-badge {
  display: flex; align-items: center; gap: 10px;
  font-family: var(--font-mono); font-size: 0.75rem; color: var(--neon-pink);
  padding: 12px 20px; border: 1px solid rgba(255,0,110,0.3); border-radius: 4px;
  background: rgba(255,0,110,0.05);
}

/* Poster thumbs */
.poster-thumbs { display: flex; flex-direction: column; gap: 8px; margin-top: 0.5rem; }
.thumbs-label { font-family: var(--font-mono); font-size: 0.6rem; letter-spacing: 0.2em; color: var(--text-muted); }
.thumbs-row { display: flex; gap: 8px; }
...
.thumb:hover, .thumb.active {
  opacity: 1; border-color: var(--neon-cyan);
...
.review-form { padding: 2rem; margin-bottom: 2rem; }
...
.review-textarea {
  ...
  background: rgba(0,245,255,0.02); border: 1px solid var(--border-subtle);
  border-radius: 4px; color: var(--text-primary); font-family: var(--font-body);
  font-size: 0.9rem; resize: vertical; outline: none; transition: border 0.3s;
  margin-bottom: 1rem;
}

.review-textarea:focus { border-color: var(--neon-cyan); }
.review-error { font-family: var(--font-mono); font-size: 0.75rem; color: var(--neon-pink); margin-bottom: 0.75rem; }
.form-actions { display: flex; justify-content: flex-end; }

.review-info {
  display: flex; align-items: center; gap: 10px;
  font-family: var(--font-mono); font-size: 0.75rem; color: var(--text-muted);
  padding: 1rem 1.5rem; border: 1px solid var(--border-subtle); border-radius: 4px;
  background: rgba(0,245,255,0.02); margin-bottom: 2rem;
}

.info-icon { color: var(--neon-cyan); }
...
.pay-summary {
  display: flex; justify-content: space-between; align-items: center;
  padding: 1rem; background: rgba(0,245,255,0.03); border: 1px solid var(--border-subtle); border-radius: 4px;
}
...
.pay-error { font-family: var(--font-mono); font-size: 0.75rem; color: var(--neon-pink); }
